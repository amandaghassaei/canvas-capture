# canvas-capture
A small wrapper around [CCapture.js](https://github.com/spite/ccapture.js) and [ffmpeg.wasm](https://github.com/ffmpegwasm/ffmpeg.wasm) to record the canvas as an image (png/jpeg), video (mp4/webm), or gif.  Video export currently only works in Chrome (see [Caveats](#caveats) for more details about browser support).  MP4 videos are generated by recording as webm, then converting into mp4 in the browser using [ffmpeg.wasm](https://github.com/ffmpegwasm/ffmpeg.wasm).

This project doesn't expose *all* the options of either CCapture.js or ffmpeg.wasm, but it packages them in a convenient way to be used via npm on the browser (I'm mostly building with typescript + webpack).  Also, I've added some helper functions to bind recording and screen-shotting to hotkeys, added an optional recording indicator on screen to let you know when recording is happening, and some other optional modal dialog features.
