# canvas-capture
A small wrapper around [CCapture.js](https://github.com/spite/ccapture.js) and [ffmpeg.wasm](https://github.com/ffmpegwasm/ffmpeg.wasm) to record the canvas as an image (png/jpeg), video (mp4/webm), or gif – all in the browser!  Video export currently only works in Chrome (see [Caveats](#caveats) for more details about browser support).  MP4 videos are generated by recording as webm, then converting into mp4 in the browser using ffmpeg.wasm.

This project doesn't expose *all* the options of either CCapture.js or ffmpeg.wasm, but it packages them in a convenient way to be installed via npm and run in the browser (I'm mostly building with typescript + webpack).  I've also added:
...
CanvasCapture.init(document.getElementById('my-canvas'), {
  ...
  // or copy the file at https://unpkg.com/browse/@ffmpeg/core@0.10.0/dist/ and
